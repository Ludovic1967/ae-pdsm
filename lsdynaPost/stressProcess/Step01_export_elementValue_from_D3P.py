...
# D_output_path = r'D:\SIMULATION\p01_DebrisCloudDamageDataBase\originalData\original_elementValue_npyFiles\damageM_Data'

VMstress_output_path = r'D:\SIMULATION\p01_DebrisCloudDamageDataBase\originalData\eval\original_elementValue_npyFiles\VMStress_Data'  # 请根据需要修改
EPstrain_output_path = r'D:\SIMULATION\p01_DebrisCloudDamageDataBase\originalData\eval\original_elementValue_npyFiles\EPStrain_Data'      # 请根据需要修改
# 如果保存路径不存在，则创建
if not os.path.exists(VMstress_output_path):
    os.makedirs(VMstress_output_path)
...
# existing_damageM_files = set(os.listdir(D_output_path))
...
for bumper_folder in os.listdir(main_path):
    ...
    for dp_folder in os.listdir(bumper_path):
        ...
        for vp_folder in os.listdir(dp_path):
            ...
            damageM_output_filename = f"Bumper{bumper_num}_dp{dp_num}_vp{vp_num}_elementMax_damageM.npy"

            # 如果输出文件已经存在，则跳过处理
            if ((VMstress_output_filename in existing_VMstress_files) and (EPstrain_output_filename in existing_EPstrain_files)):
                    # and (damageM_output_filename in existing_damageM_files)):
                print(f"Skip already processed: {VMstress_output_filename} & {EPstrain_output_filename} & {damageM_output_filename}")
                continue
            ...
            # 打印结果信息
            print(f"element_Max_VonMisesStress.shape: {element_Max_VonMisesStress.shape}")
            print(f"最小值: {np.min(element_Max_VonMisesStress)}, 最大值: {np.max(element_Max_VonMisesStress)}")
            ...
            # 保存结果
            np.save(VMstress_output_file, element_Max_VonMisesStress)
            print(f"Saved Max Von Mises Stress to {VMstress_output_file}\n")

            np.save(EPstrain_output_file, element_Max_eps)
            print(f"Saved Max EP Strain to {EPstrain_output_file}\n")

            # 释放内存
            del element_VonMisesStress_all
            del element_EffectivePlasticStrain_all
            del element_Max_VonMisesStress
            del element_Max_eps
            del element_VonMisesStress
            del element_eps
            del dr
            gc.collect()
